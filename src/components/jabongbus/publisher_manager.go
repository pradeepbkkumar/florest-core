...
func (pub *jPublish) PublishMessage(preq *PubRequest, timeOut time.Duration, debug bool) (ret *PubResponse, err error) {
	...
	headers[constants.USER_ID] = preq.Headers.UserId
	headers[constants.SESSION_ID] = preq.Headers.SessionId
	headers[constants.TRANSACTION_ID] = preq.Headers.TransactionId
	headers[constants.REQUEST_ID] = preq.Headers.RequestId
	// get return object
	ret = new(PubResponse)
